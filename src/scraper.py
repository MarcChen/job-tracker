...
def scrape_all_offers(driver, include_filters, exclude_filters, debug=False):
    ...
    url_wtj="https://www.welcometothejungle.com/fr/jobs?refinementList[offices.country_code][]=FR&query=data",
    ...
    with Progress(
        ...
        task_wtj = progress.add_task("[yellow]Scraping Welcome to the Jungle offers...", total=None)
        data_wtj_data_engineer = scraper_wtj_data_engineer.scrape()
        progress.remove_task(task_wtj)

        task_wtj_ai = progress.add_task("[yellow]Scraping Welcome to the Jungle offers...", total=None)
    ...
    offers = data_VIE["offers"] + data_Air_France["offers"] + data_apple["offers"] + data_wtj_data_engineer["offers"] + data_wtj_ai["offers"]
